...
from yogurt import shell, daemon, eventloop, tcprelay, udprelay, \
    asyncdns, manager, common

import yogurt.crypto.siphash
...
def main():
    ...
    logging.info("IPv6 is %ssupported" % ("" if asyncdns.IPV6_CONNECTION_SUPPORT else "not "))
    sipmod = yogurt.crypto.siphash._siphashc
    if sipmod:
        logging.info("Fast siphash module loaded: %s" % sipmod.__file__)
    else:
        logging.warn("Fallback to slow siphash implementation")
    ...
    tcp_servers = []
    udp_servers = []
    dns_resolver = asyncdns.DNSResolver(config.get('dns', None))
