...
import getopt
import logging
from yogurt.common import to_bytes, to_str, IPNetwork, PortRange
from yogurt import encrypt
...
def __version():
    ...
    try:
        ...
        version_str = pkg_resources.get_distribution('yogurt').version
    except Exception:
        try:
            from yogurt import version
...
def get_config(is_local):
    ...
    config['pid-file'] = config.get('pid-file', '/var/run/yogurtr.pid')
    config['log-file'] = config.get('log-file', '/var/log/yogurtr.log')
    ...
    if is_local:
        ...
        try:
            config['forbidden_ip'] = \
                IPNetwork(config.get('forbidden_ip', ''))
...
Online help: <https://github.com/yogurt/yogurt>
''')
