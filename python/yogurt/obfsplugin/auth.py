...
import yogurt
from yogurt import common, lru_cache, encrypt
from yogurt.obfsplugin import plain
from yogurt.common import to_bytes, to_str, ord, chr
from yogurt.crypto import siphash

def create_auth_sha1_v4(method):
    return auth_sha1_v4(method)

def md5_hmac_new(key, msg):
    return hmac.new(key, msg, hashlib.md5)

def sha1_hmac_new(key, msg):
    return hmac.new(key, msg, hashlib.sha1)

def md5_hash(key, msg):
    return hashlib.md5(msg)

def sha1_hash(key, msg):
    return hashlib.sha1(msg)

def create_auth_aes128_md5(method):
    return auth_aes128_sha1(method, md5_hash, md5_hmac_new, b'auth_aes128_md5')

def create_auth_aes128_sha1(method):
    return auth_aes128_sha1(method, sha1_hash, sha1_hmac_new, b'auth_aes128_sha1')

def create_auth_aes128_fast(method):
    return auth_aes128_sha1(method, siphash.new, siphash.new, b'auth_aes128_fast')
...
obfs_map = {
        ...
        'auth_aes128_fast': (create_auth_aes128_fast,),
}
...
class auth_aes128_sha1(auth_base):
    def __init__(self, method, hashfunc, hmac_new, salt):
        super(auth_aes128_sha1, self).__init__(method)
        self.hashfunc = hashfunc
        self.hmac_new = hmac_new
        self.recv_buf = b''
        self.unit_len = 8100
        self.raw_trans = False
        self.has_sent_header = False
        ...
        self.max_time_dif = 60 * 60 * 24 # time dif (second) setting
        self.salt = salt
        self.no_compatible_method = salt
    ...
    def set_server_info(self, server_info):
        ...
        try:
            ...
            max_client = 64
        self.server_info.data.set_max_client(max_client)
        self.hashkey = self.server_info.key
        if (len(self.hashkey) < 16):
            self.hashkey += b'\x00' * (16 - len(self.hashkey))
        self.hashkey = self.hashkey[:16]
    ...
    def pack_data(self, buf, full_buf_size):
        ...
        mac = self.hmac_new(mac_key, struct.pack('<H', data_len)).digest()[:2]
        data = struct.pack('<H', data_len) + mac + data
        data += self.hmac_new(mac_key, data).digest()[:4]
    ...
    def pack_auth_data(self, auth_data, buf):
        ...
        if b':' in to_bytes(self.server_info.protocol_param):
            try:
                ...
                self.user_key = self.hashfunc(self.hashkey, items[1]).digest()
        ...
        check_head += self.hmac_new(mac_key, check_head).digest()[:6]
        data = check_head + data + os.urandom(rnd_len) + buf
        data += self.hmac_new(self.user_key, data).digest()[:4]
        return data
    ...
    def client_post_decrypt(self, buf):
        ...
        while len(self.recv_buf) > 4:
            ...
            mac = self.hmac_new(mac_key, self.recv_buf[:2]).digest()[:2]
            ...
            if self.hmac_new(mac_key, self.recv_buf[:length - 4]).digest()[:4] != self.recv_buf[length - 4:length]:
                self.raw_trans = True
                self.recv_buf = b''
                raise Exception('client_post_decrypt data uncorrect checksum')
    ...
    def server_post_decrypt(self, buf):
        ...
        if not self.has_recv_header:
            if len(self.recv_buf) >= 7 or len(self.recv_buf) in [2, 3]:
                ...
                sha1data = self.hmac_new(mac_key, self.recv_buf[:1]).digest()[:recv_len - 1]
                if sha1data != self.recv_buf[1:recv_len]:
                    return self.not_match_return(self.recv_buf)

            if len(self.recv_buf) < 31:
                return (b'', False)
            sha1data = self.hmac_new(mac_key, self.recv_buf[7:27]).digest()[:4]
            ...
            if uid in self.server_info.users:
                self.user_id = uid
                self.user_key = self.hashfunc(self.hashkey, self.server_info.users[uid]).digest()
            ...
            if self.hmac_new(self.user_key, self.recv_buf[:length - 4]).digest()[:4] != self.recv_buf[length - 4:length]:
    ...
    def client_udp_pre_encrypt(self, buf):
        if self.user_key is None:
            ...
            if self.user_key is None:
                ...
                self.user_key = self.server_info.key
        buf += self.user_id
        return buf + self.hmac_new(self.user_key, buf).digest()[:4]

    def client_udp_post_decrypt(self, buf):
        user_key = self.server_info.key
        if self.hmac_new(user_key, buf[:-4]).digest()[:4] != buf[-4:]:
            return b''
        return buf[:-4]

    def server_udp_pre_encrypt(self, buf, uid):
        user_key = self.server_info.key
        return buf + self.hmac_new(user_key, buf).digest()[:4]

    def server_udp_post_decrypt(self, buf):
        uid = buf[-8:-4]
        if uid in self.server_info.users:
            user_key = self.hashfunc(self.hashkey, self.server_info.users[uid]).digest()
        else:
            uid = None
            if not self.server_info.users:
                user_key = self.server_info.key
