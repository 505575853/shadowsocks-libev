...
sds CryptoRandBytes(int length) {
    sds buf = make_sds(length);
    rand_bytes((uint8_t *)buf, length);
    return buf;
}

// PsudoRandBytes returns a byte slice filled with psudorandom bytes generated by the seed
sds PsudoRandBytes(int len, int64_t seed) {
...
sds DecodeHexString(const char *s) {
...
sds AddRecordLayer(sds input, uint8_t typ[1], uint8_t ver[2]) {
...
sds makeSessionTicket(State *sta) {
...
sds addExtRec(uint8_t typ[2], sds data) {
    sds length = make_sds(2);
...
void encrypt(uint8_t iv[16], uint8_t key[SHA256_BYTES], 
...
uint8_t *makeGREASEChrome(uint8_t data[2]) {
...
sds composeExtensionsChrome(State* sta) {
...
sds composeClientHelloChrome(State* sta) {
    sds clientHello[12];
...
sds ComposeInitHandshake(State* sta) {
    sds ch;
